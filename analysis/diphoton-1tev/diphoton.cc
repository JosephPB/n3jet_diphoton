...
public:
    ...
    void analyze(const Event& event)
    {
        ...
        const shared_ptr<fastjet::ClusterSequenceArea> clust_seq_area = applyProjection<FastJets>(event, "AntiKtJetsD05").clusterSeqArea();
        for (const fastjet::PseudoJet& jet : apply<FastJets>(event, "AntiKtJetsD05").pseudoJets(0.0 * GeV)) {
        ...
        const std::vector<fastjet::PseudoJet> jets = fastjet::sorted_by_E(apply<FastJets>(event, "AntiKtJetsD05").pseudoJets(0.0 * GeV));
        ...
        for (const Particle& photon : photons) {
            ...
            // Compute isolation via particles within an R=0.4 cone of the photon (IsolationCut?)
            ...
            for (const Particle& p : fs) {
                ...
                //if (fabs(eta_P - p.eta()) < 0.025 * 5 * 0.5 && fabs(phi_P - p.phi()) < PI / 128. * 7 * 0.5)
                //    continue;
                // Sum momentum
            ...
            // Discard the photon if there is more than 11 GeV of cone activity - removed as detector cuts
...
private:
    Histo1DPtr _h_M, _h_pT, _h_dphiyy, _h_dphijj, _h_costh, _h_phistar, _h_at, _h_rsepjy, _h_j1pt, _h_j2pt, _h_etayy, _h_etajj;
